...
This version implements parallel processing and writes the results to a temporary file to reduce IO overhead.
"""
...
def parse_opt():
    ...
    parser.add_argument('--real-fps', '-rfps', type=int or float, default=None,
                        help='Enter the framerate you were using (only required if interpolation is turned on).')
    parser.add_argument('--max-interpolation-seconds', '-mis', type=int or float, default=None,
                        help='Maximum number of seconds to interpolate between frames (only required if interpolation is turned on).')
...
def process_file(file_path, opt, funcs, temp_dir):
    """
    Process a single file and write the analysis results to a temporary file.
    ...
    # Write the result to a temporary file on the local machine
    temp_filename = os.path.join(temp_dir, f"temp_{uuid.uuid4().hex}.csv")
    oneVid.to_csv(temp_filename, index=False)
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        if opt['cores'] > 1:
            ...
            with Pool(processes=opt['cores']) as pool:
                results = pool.starmap(process_file, [(file_path, opt, funcs, tmpdirname) for file_path in file_list])
                successful_results = [r for r in results if r is not None]
                print(f"Successfully processed {len(successful_results)} out of {len(results)} files.")

        else:
            print("Processing files serially...")
            results = [process_file(file_path, opt, funcs, tmpdirname) for file_path in file_list]
            successful_results = [r for r in results if r is not None]
            print(f"Successfully processed {len(successful_results)} out of {len(results)} files.")
        ...
        if temp_files:
            ...
            combined = pd.concat(df_list, ignore_index=True, axis=0)
